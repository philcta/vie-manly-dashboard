...
/** Pills shown directly in the toolbar (desktop only) */
...
export default function PeriodSelector({
    ...
    // All selectable periods for the mobile dropdown (pills + dropdown options)
    const allSelectablePeriods = periods.filter((p) => p !== "custom");

    return (
        <div className="space-y-2">
            <div className="flex items-center gap-3 flex-wrap">

                {/* ── MOBILE: Single dropdown for period selection ── */}
                {isMobile ? (
                    <div className="flex items-center gap-2 w-full" ref={customRef}>
                        <div className="relative flex-1">
                            <select
                                value={period === "custom" ? "__custom__" : period}
                                onChange={(e) => {
                                    const val = e.target.value;
                                    if (val === "__custom__") {
                                        setCustomOpen(true);
                                    } else {
                                        onPeriodChange(val as PeriodType);
                                        setCustomOpen(false);
                                    }
                                }}
                                className="w-full px-4 py-2.5 text-sm font-medium rounded-xl border border-border bg-card text-foreground appearance-none cursor-pointer focus:outline-none focus:ring-2 focus:ring-olive/20 pr-10"
                            >
                                {allSelectablePeriods.map((p) => (
                                    <option key={p} value={p}>{periodLabels[p]}</option>
                                ))}
                                {hasCustom && (
                                    <option value="__custom__">
                                        {period === "custom" ? `Custom: ${formatDateRange(currentRange.startDate, currentRange.endDate)}` : "Custom range…"}
                                    </option>
                                )}
                            </select>
                        ...
                        {showComparison && (
                            <div className="relative flex-1">
                                <select
                                    ...
                                    className="w-full px-4 py-2.5 text-sm rounded-xl border border-border bg-card text-foreground appearance-none cursor-pointer focus:outline-none focus:ring-2 focus:ring-olive/20 pr-10"
                                >
                        ...
                        {/* Custom date picker panel (mobile) */}
                        {customOpen && (
                            <div className="absolute left-3 right-3 top-full mt-2 z-50 bg-card rounded-xl border border-border shadow-lg overflow-visible"
                                ...
                                <div className="p-4 space-y-3">
                                    ...
                                    <div className="flex gap-2">
                                        <button
                                            ...
                                            className="flex-1 py-2 text-sm font-medium rounded-lg border border-border text-muted-foreground hover:bg-muted/50 cursor-pointer transition-colors"
                                        >
                                            Cancel
                                        </button>
                                        <button
                                            ...
                                            className="flex-1 py-2 text-sm font-semibold rounded-lg bg-olive text-white hover:bg-olive/90 disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer transition-colors"
                                        >
                                            Apply
                                        </button>
                                    </div>
                    ...
                    /* ── DESKTOP: Pill strip + More dropdown ── */
                    <>
                        {/* Period pills */}
