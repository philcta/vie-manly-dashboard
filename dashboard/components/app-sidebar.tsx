"use client";

import Link from "next/link";
import { usePathname } from "next/navigation";
import { useState, useEffect } from "react";
import {
    LayoutDashboard,
    Users,
    Package,
    UserCog,
    MessageSquare,
    Settings,
    ChevronsLeft,
    ChevronsRight,
} from "lucide-react";

const navItems = [
    { href: "/", label: "Overview", icon: LayoutDashboard },
    { href: "/members", label: "Members", icon: Users },
    { href: "/inventory", label: "Inventory", icon: Package },
    { href: "/staff", label: "Staff", icon: UserCog },
    { href: "/campaigns", label: "SMS Campaigns", icon: MessageSquare },
    { href: "/settings", label: "Settings", icon: Settings },
];

export default function AppSidebar() {
    const pathname = usePathname();
    const [collapsed, setCollapsed] = useState(false);

    // Persist collapse state + auto-collapse on small screens
    useEffect(() => {
        const mql = window.matchMedia("(max-width: 1023px)");

        const applyState = () => {
            if (mql.matches) {
                // Small screen — force collapsed
                setCollapsed(true);
            } else {
                // Large screen — restore saved preference
                const saved = localStorage.getItem("sidebar-collapsed");
                setCollapsed(saved === "true");
            }
        };

        applyState();
        mql.addEventListener("change", applyState);
        return () => mql.removeEventListener("change", applyState);
    }, []);

    const toggle = () => {
        setCollapsed((prev) => {
            const next = !prev;
            // Defer side-effects to avoid "setState during render" in DashboardShell
            requestAnimationFrame(() => {
                localStorage.setItem("sidebar-collapsed", String(next));
                window.dispatchEvent(new Event("sidebar-toggle"));
            });
            return next;
        });
    };

    const width = collapsed ? "w-[56px]" : "w-[220px]";

    return (
        <aside
            className={`fixed top-0 left-0 h-screen ${width} bg-[#1E1E2E] hidden md:flex flex-col z-50 transition-all duration-300 ease-in-out`}
        >
            {/* Logo */}
            <div className="px-3 py-6 flex flex-col items-center border-b border-white/6">
                {collapsed ? (
                    <h1 className="text-[#A8B094] font-serif text-lg font-bold tracking-wide">
                        V<span className="text-[#6B7355]">.</span>
                    </h1>
                ) : (
                    <>
                        <h1 className="text-[#A8B094] font-serif text-2xl font-bold tracking-wide">
                            VIE<span className="text-[#6B7355]">.</span>
                        </h1>
                        <span className="text-[#7A7A8A] text-[10px] uppercase tracking-[0.3em] mt-0.5">
                            M A N L Y
                        </span>
                    </>
                )}
            </div>

            {/* Navigation */}
            <nav className="flex-1 py-4 px-2 space-y-1 overflow-y-auto overflow-x-hidden">
                {navItems.map((item) => {
                    const isActive =
                        pathname === item.href ||
                        (item.href !== "/" && pathname.startsWith(item.href));

                    return (
                        <Link
                            key={item.href}
                            href={item.href}
                            title={collapsed ? item.label : undefined}
                            className={`
                                group flex items-center gap-3 rounded-lg text-sm font-medium
                                transition-all duration-200 cursor-pointer whitespace-nowrap overflow-hidden
                                ${collapsed ? "px-2.5 py-2.5 justify-center" : "px-3 py-2.5"}
                                ${isActive
                                    ? "text-[#A8B094] bg-[rgba(107,115,85,0.08)] border-l-[3px] border-[#6B7355]"
                                    : "text-[#B8B8C8] hover:text-white hover:bg-white/[0.03] border-l-[3px] border-transparent"
                                }
                            `}
                        >
                            <item.icon
                                className={`w-[18px] h-[18px] flex-shrink-0 ${isActive ? "text-[#6B7355]" : "text-[#7A7A8A] group-hover:text-[#B8B8C8]"
                                    }`}
                            />
                            {!collapsed && <span>{item.label}</span>}
                        </Link>
                    );
                })}
            </nav>

            {/* Toggle + bottom info */}
            <div className="border-t border-white/6">
                <button
                    onClick={toggle}
                    className="w-full flex items-center justify-center gap-2 py-3.5 text-[#B8B8C8] hover:text-white hover:bg-white/[0.04] transition-all duration-200 cursor-pointer"
                    title={collapsed ? "Expand sidebar" : "Collapse sidebar"}
                >
                    {collapsed ? (
                        <ChevronsRight className="w-[18px] h-[18px]" />
                    ) : (
                        <>
                            <ChevronsLeft className="w-[18px] h-[18px]" />
                            <span className="text-xs font-medium">Collapse</span>
                        </>
                    )}
                </button>
                {!collapsed && (
                    <p className="text-[10px] text-[#5A5A6A] text-center pb-3">
                        Vie Market &amp; Bar
                    </p>
                )}
            </div>
        </aside>
    );
}
